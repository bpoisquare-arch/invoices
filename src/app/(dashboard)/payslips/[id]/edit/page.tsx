...
import { AIMTPayslip, aimtPayslipService } from '@/lib/services/aimt-payslip.service'
...
export default function EditPayslipPage({ params }: EditPayslipPageProps) {
  ...
  useEffect(() => {
    async function fetchPayslip() {
      ...
      setLoading(true)
      try {
        const data = await aimtPayslipService.getById(resolvedParams.id)
        if (data) {
          setPayslip(data)
        } else {
  ...
  return <AIMTPayslipForm initialData={payslip} isEditing={true} />
}
