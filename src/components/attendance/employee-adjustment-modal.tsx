...
interface EmployeeAdjustmentModalProps {
  ...
  currentNotes?: string
  onSaveSuccess?: (updatedAdjustment: { month: string; amount: number; notes: string }) => void
}
...
export default function EmployeeAdjustmentModal({
  ...
  const [historyList, setHistoryList] = useState<AdjustmentRecord[]>([])
  ...
  // Computed period key e.g. "2026-09"
  ...
  const fetchEmployeeHistory = async () => {
    ...
    try {
      const res = await fetch(`/api/attendance/adjustments?employeeId=${encodeURIComponent(employee.id)}`)
      const data = await res.json()
      if (data.success && Array.isArray(data.adjustments)) {
        setHistoryList(data.adjustments)
        // Check if there is already an adjustment for the currently selected month
        const match = data.adjustments.find((a: AdjustmentRecord) => a.month_year === selectedMonthKey)
  ...
  const handlePeriodChange = (newYear: string, newMonth: string) => {
    ...
    const match = historyList.find((a) => a.month_year === newKey)
    if (match) {
  ...
  const handleEditFromHistory = (item: AdjustmentRecord) => {
  ...
  const handleDeleteRecord = async (monthYearToDelete: string) => {
    ...
    const confirmDelete = window.confirm(
      `Are you sure you want to delete the adjustment record for ${formatMonthLabel(monthYearToDelete)}?`
    )
    ...
    try {
      const res = await fetch(
        `/api/attendance/adjustments?employeeId=${encodeURIComponent(employee.id)}&month=${encodeURIComponent(
          monthYearToDelete
        )}`,
        { method: 'DELETE' }
      )
      ...
      setSuccessMsg(`Adjustment for ${formatMonthLabel(monthYearToDelete)} deleted successfully.`)
      setHistoryList((prev) => prev.filter((a) => a.month_year !== monthYearToDelete))
      ...
      setError(err.message || 'Error deleting adjustment.')
    } finally {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMsg(`Adjustment of PKR ${parsedAmt.toLocaleString()} saved for ${formattedLabel}!`)

      // Update or insert into history list
      setHistoryList((prev) => {
        const idx = prev.findIndex((a) => a.month_year === selectedMonthKey)
        const updatedEntry: AdjustmentRecord = {
          id: data.adjustment?.id || `adj_${Date.now()}`,
  ...
  const totalWithAdjustment = baseSalary + numAmount
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="w-[95vw] sm:max-w-2xl md:max-w-4xl max-h-[90vh] bg-white border border-slate-200 shadow-2xl rounded-2xl p-4 sm:p-5 font-sans flex flex-col overflow-y-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-3.5 pt-1">
            ...
            <div className="space-y-2.5">
              <div className="flex items-center justify-between border-b border-slate-200/60 pb-1.5">
                <h3 className="text-[11px] font-bold text-slate-800 uppercase tracking-wider flex items-center gap-1.5">
                  ...
                  {editingRecordId ? 'Edit Monthly Adjustment' : 'Set Monthly Adjustment'}
                </h3>
                {editingRecordId && (
                  <button
                    type="button"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                <div className="space-y-1">
                  ...
                  <div className="relative">
                    ...
                    <Input
                      ...
                      className="pl-11 text-xs font-bold font-mono border-slate-200 h-8 bg-white focus:border-cyan-600"
                      required
                    />
                  </div>
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <Input
                    ...
                    placeholder="e.g. Salary Arrears / Bonus"
              ...
              <div className="bg-white border border-cyan-200/80 rounded-lg p-2.5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-1.5 sm:gap-2 text-xs shadow-2xs">
                <div className="flex items-center gap-1.5 min-w-0 flex-wrap">
                  <span className="text-[10px] font-bold px-1.5 py-0.5 bg-cyan-100 text-cyan-900 border border-cyan-300 rounded shrink-0 font-mono">
                    {currentSelectedLabel}
                  </span>
                  <span className="text-[11px] text-slate-600">
                    Base <strong className="text-slate-900 font-bold">{baseSalary.toLocaleString()}</strong> + Adj <strong className="text-cyan-800 font-bold">+{numAmount.toLocaleString()}</strong>
                  </span>
                </div>
                <div className="flex items-center gap-1 font-bold shrink-0 self-end sm:self-auto">
                  ...
                  <span className="text-emerald-700 text-xs font-mono font-extrabold">
                    PKR {totalWithAdjustment.toLocaleString()}
                  </span>
                </div>
              </div>
            </div>
              ...
              >
                {isSaving ? (
                  ...
                  <>
                    ...
                    {editingRecordId ? `Update (${selectedMonthKey})` : `Add Adjustment (${selectedMonthKey})`}
                  </>
                )}
              </Button>
            </div>
          ...
          {/* Right Column: Adjustment History (5 cols) */}
          <div className="md:col-span-5 flex flex-col bg-slate-50/60 border border-slate-200/80 rounded-xl p-3">
            ...
            {isLoadingHistory ? (
              ...
              <div className="py-8 px-2 text-center text-slate-400 text-xs flex flex-col items-center justify-center space-y-1">
                ...
                <p className="text-[10px] text-slate-400">Recorded adjustments will appear here.</p>
              </div>
            ) : (
                    ...
                    >
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="font-mono font-extrabold text-cyan-800 text-xs truncate">
