...
export default function STCResendEmailDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[620px] bg-[#001724] border border-white/10 text-white shadow-2xl p-6 rounded-2xl">
        <DialogHeader className="space-y-1">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg bg-[#00BF8F]/20 text-[#00BF8F] flex items-center justify-center">
        ...
        <div className="p-3.5 rounded-xl bg-[#001E2F] border border-white/10 space-y-1.5">
          <div className="flex items-center justify-between text-xs">
            <span className="font-semibold text-slate-300 flex items-center gap-1.5">
              <Clock className="w-3.5 h-3.5 text-[#00BF8F]" />
              Resend Quota:
            </span>
            <span
              className={`font-bold font-mono ${
                eligibility.allowed ? 'text-[#00BF8F]' : 'text-rose-400'
              }`}
            >
        ...
        <form onSubmit={handleSendEmail} className="space-y-4">
          <div className="space-y-1.5">
            <Label className="text-xs font-semibold text-slate-300">From Verified Sender</Label>
            <select
              value={fromEmail}
            ...
            >
              {STC_VERIFIED_SENDERS.map((s) => (
                <option key={s.email} value={s.email} className="bg-[#001E2F] text-white">
                  {s.label}
                </option>
              ))}
            </select>
          ...
          <div className="space-y-1.5">
            ...
            <Textarea
              ...
              className="bg-[#001E2F] border-white/15 text-white text-xs rounded-xl focus:border-[#00BF8F] resize-none"
            />
          </div>
          ...
          <DialogFooter className="pt-2 flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <Button
                ...
                className="bg-[#00BF8F] hover:bg-[#00a87e] text-[#001E2F] rounded-xl text-xs font-bold cursor-pointer"
              >
                {isSending ? (
                  <>
        ...
        {showLogs && (
          <div className="mt-4 pt-4 border-t border-white/10 max-h-48 overflow-y-auto space-y-2">
            <p className="text-[11px] font-bold uppercase tracking-wider text-slate-400">
              Recent Dispatches
            </p>
            {logs.length === 0 ? (
              <p className="text-xs text-slate-500 italic">No email logs found for this schedule.</p>
            ) : (
              logs.map((l) => (
                <div
                  key={l.id}
                  className="p-2.5 rounded-lg bg-[#001E2F] border border-white/5 text-xs flex items-center justify-between"
                >
                  <div>
                    <span className="font-semibold text-slate-200">{l.to_email}</span>
