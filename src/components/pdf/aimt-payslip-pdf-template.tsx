...
import { AIMTPayslip, formatCurrency } from '@/lib/services/aimt-payslip.service'
import { AIMT_LOGO_BASE64 } from '@/lib/constants/aimt-assets'
...
const styles = StyleSheet.create({
  ...
  // Borderless flat gray cards matching Attachment 2
  paidByBox: {
    width: 235,
...
interface AIMTPayslipPDFTemplateProps {
  payslip: AIMTPayslip
}

export default function AIMTPayslipPDFTemplate({ payslip }: AIMTPayslipPDFTemplateProps) {
  ...
  return (
    <Document>
      <Page size="A4" style={styles.page}>
        {/* 1. Header: Logo & Paid By (Matching Attachment 2) */}
        <View style={styles.headerGrid}>
          <View style={styles.logoContainer}>
            <Image src={AIMT_LOGO_BASE64} style={styles.logoImage} />
          </View>
          <View style={styles.paidByBox}>
            <Text style={styles.paidByTitle}>PAID BY</Text>
            <Text style={styles.paidByText}>{payslip.paid_by_name || 'Australian Institute of Management and Technology'}</Text>
            <Text style={styles.paidByText}>{payslip.paid_by_address_1 || '84 Buckley Street'}</Text>
            <Text style={styles.paidByText}>{payslip.paid_by_address_2 || 'Footscray VIC 3011'}</Text>
            <Text style={styles.paidByText}>ABN {payslip.paid_by_abn || '85 136 626 956'}</Text>
          </View>
        </View>

        {/* 2. Middle Section: Employee Address & Employment Details (Matching Attachment 2) */}
        <View style={styles.middleGrid}>
          <View style={styles.employeeBox}>
            <Text style={styles.employeeName}>{payslip.employee_name || 'Ubaid Raza'}</Text>
        ...
        <View style={styles.sectionContainer}>
          ...
          <View style={styles.tableDataRow}>
            ...
            <Text style={styles.colPayRef}>{payslip.payment_reference || 'AIMT Pay'}</Text>
