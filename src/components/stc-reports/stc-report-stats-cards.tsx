...
import { Users, DollarSign, Clock, AlertCircle, Sparkles } from 'lucide-react'

interface StcReportStatsCardsProps {
...
export default function StcReportStatsCards({
  totalStudents,
  totalPendingAmount,
  pendingInvoicesCount,
  totalYetToRaised,
  selectedCount = 0,
}: StcReportStatsCardsProps) {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
      {/* 1. Total Students */}
      <Card className="relative bg-gradient-to-b from-white to-slate-50/80 border border-slate-200/80 text-slate-900 shadow-xs hover:shadow-md hover:border-[#00BF8F]/40 hover:-translate-y-0.5 transition-all duration-200 rounded-2xl overflow-hidden group">
        <div className="absolute top-0 inset-x-0 h-1 bg-gradient-to-r from-[#00BF8F] to-[#008f6b]" />
        <CardContent className="p-5 flex items-center justify-between">
          <div className="space-y-1">
            <div className="flex items-center gap-1.5">
              <span className="size-1.5 rounded-full bg-[#00BF8F] animate-pulse" />
            ...
            <div className="pt-0.5">
              {selectedCount > 0 ? (
                <span className="inline-flex items-center gap-1 text-[10.5px] font-bold text-[#006e52] bg-emerald-50 px-2 py-0.5 rounded-md border border-emerald-200">
                  <Sparkles className="size-3 text-[#00BF8F]" />
          ...
          <div className="size-12 rounded-2xl bg-gradient-to-br from-[#00BF8F] to-[#001E2F] text-white flex items-center justify-center shadow-md shadow-[#00BF8F]/20 shrink-0 group-hover:scale-105 transition-transform">
      ...
      <Card className="relative bg-gradient-to-b from-white to-emerald-50/30 border border-slate-200/80 text-slate-900 shadow-xs hover:shadow-md hover:border-emerald-300 hover:-translate-y-0.5 transition-all duration-200 rounded-2xl overflow-hidden group">
        ...
        <CardContent className="p-5 flex items-center justify-between">
          ...
          <div className="size-12 rounded-2xl bg-gradient-to-br from-[#00BF8F] to-teal-700 text-white flex items-center justify-center shadow-md shadow-[#00BF8F]/20 shrink-0 group-hover:scale-105 transition-transform">
