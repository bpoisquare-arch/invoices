...
import NSCWebPreview from '@/components/templates-preview/nsc-web-preview'
...
export function renderInvoiceWebPreview(invoice: Partial<InvoiceWithDetails>, snapshot?: TemplateSnapshot) {
  ...
  switch (layout) {
    case 'nsc_v1':
      return <NSCWebPreview invoice={invoice} snapshot={snapshot} />
    case 'anonymous_v1':
...
export function renderInvoicePDFDocument(
  ...
  switch (layout) {
    case 'nsc_v1':
      return <NSCPDFTemplate invoice={invoice} snapshot={snapshot} resolvedLogoUrl={resolvedLogoUrl} />
    case 'anonymous_v1':
